...
class ScheduleContextGenerator:
    """Class to generate context about Alexa's current activity based on schedules."""
    ...
    def get_current_activity(cls) -> Optional[str]:
        """
        Get Alexa's current activity based on the current time and day of the week.

        Returns:
            str: Description of current activity, or None if no matching time slot is found 
        """

        # get current time and day of the week (0 = Monday, 6 = Sunday)
        current_datetime = datetime.now()
        current_time = current_datetime.time()
        current_day = current_datetime.weekday()  # give 0 - 6 for monday - sunday 
    ...
    def get_schedule_for_day(cls, day: int) -> Dict[str, str]:
        """Get the complete scehdule for a specific day.
